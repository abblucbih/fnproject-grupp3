...
<q-page class="flex flex-center">
  <section id="header" style="width: 100%; top: 0; ">
    ...
    <div class="postWindow">
      ...
      <div class="post" v-for="fish in fishes" :key="fish.id">
        <h3>{{ fish.title }}</h3>
        <h6>{{ fish.description }}</h6>
        <img
         :src="fish.path"
          ...
          <button v-if="user" @click="bookMark(fish.id,user.id )"><img
              src="https://cdn-icons-png.flaticon.com/512/1160/1160705.png" style="width: 2vw;" /></button>
              </div>
      </div>
    </div>
  </section>
...
const getFish = async () => {
  try {
    const api = await axios.get('http://localhost:3000/fish')
    fishes.value = api.data
  } catch (error) {
...
const bookMark = async (postId,userId) => {
  ...
  await axios.post('http://localhost:3000/book', { postId: postId, userId: userId, table:"fish" })
  } catch(error){
    console.log(error.message)
  }
}
